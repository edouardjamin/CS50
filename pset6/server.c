...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            // break the request-line into token
            char *method, *url, *httpvrs;
            char token[needle - haystack + 2 + 1];
            strcpy(token, line);
            method = strtok(token, " ");
            url = strtok(NULL, " ");
            httpvrs = strtok(NULL, " ");
            ...
            if (method == NULL || url == NULL || httpvrs == NULL)
            {
                error(400);
                continue;
            }
            ...
            if (strcmp(httpvrs, "HTTP/1.1\r\n") != 0)
            {
                error(505);
                continue;
            }

            // URL should contain a dot
            if (strstr(url, ".") == NULL)
            {
                error(501);
                printf("dot\n");
            ...
            char *filePath, *fullQuery;
            char queryTk[strlen(url)];
            strcpy(queryTk, url);
            filePath = strtok(queryTk, "?");
            fullQuery = strtok(NULL, "?");
            ...
            {
                isquery = true;
                strcpy(query, fullQuery);
            }

            // concatenate root and absolute-path
            char croot[] = "/home/jharvard/Dropbox/CS50/pset6/pset6/public";
            ...
            char *fileName, *ext;
            char extTk[strlen(filePath)];
            strcpy(extTk, filePath);
            fileName = strtok(extTk, ".");
            ext = strtok(NULL, ".");
